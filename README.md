...
`.agent/workflows/` ディレクトリにAI対応エディタ（Antigravity等）向けのワークフローファイルが含まれています。
必要に応じて、コピーして使用してください。

| ファイル | 説明 |
|----------|------|
| `use-grid.md` | グリッドレイアウトの使用方法 |
| `use-modal.md` | モーダルダイアログの実装方法 |
| `use-accordion.md` | アコーディオンの実装方法 |
| `use-tabs.md` | タブ切り替えの実装方法 |
| `use-forms.md` | フォーム要素のスタイリング方法 |
| `use-file-uploader.md` | ファイルアップローダーの実装方法 |
| `use-popover.md` | ポップオーバー・ツールチップの実装方法 |
| `use-utilities.md` | ユーティリティクラスとJS関数 |
| `use-alerts.md` | アラート・トースト通知の実装方法 |

これらのワークフローは `/use-grid` のようなスラッシュコマンドで呼び出せます。
